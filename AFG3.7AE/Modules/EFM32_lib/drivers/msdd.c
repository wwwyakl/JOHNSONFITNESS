...
#define ACTIVITY_LED    gpioPortE,1
...
STATIC_UBUF( cbw, USB_MAX_EP_SIZE );    /* Storage for one CBW */
static MSDBOT_CBW_TypeDef *pCbw = (MSDBOT_CBW_TypeDef*)&cbw;

EFM32_ALIGN(4)
static MSDBOT_CSW_TypeDef csw;          /* Storage for one CSW */
...
static void ProcessScsiCdb( void )
{
  ...
  EFM32_ALIGN(4)
  static MSDSCSI_ReadCapacityData_TypeDef ReadCapData;
