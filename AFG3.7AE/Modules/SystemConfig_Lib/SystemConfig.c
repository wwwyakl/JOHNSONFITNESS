...
#define FLASH_CUSTOMTIME_PRG_ADDRESS            0x3B000
#define FLASH_CUSTOMDISTANCE_PRG_ADDRESS        0x3B280
#define FLASH_CUSTOMHR_PRG_ADDRESS              0x3B500
#define FLASH_WORTOUT_INFO_ADDRESS              0x3B800
#define FLASH_MYFIRST5K_INFO_ADDRESS            0x3C000
...
#define SIZE_OF_USERINFO  676
typedef union {
    USERS_INFO ufields;
    UINT8 userInfoBuffer[SIZE_OF_USERINFO];
}USERS_INFO_UNION;

#define SIZE_OF_CUSTOMCONFIG  256//64
typedef union{
    CUSTOM_CONFIG ccfields;
    UINT8 customProgramBuffer[SIZE_OF_CUSTOMCONFIG];
}CUSTOM_CONFIG_UNION;

#define SIZE_OF_MYFIRST5KCONFIG  16
typedef union{
    MYFIRST5K_CONFIG MYfields;
    UINT8 myfirst5kProgramBuffer[SIZE_OF_MYFIRST5KCONFIG];
}MYFIRST5K_CONFIG_UNION;
...
{
    struct{
        MACHINE_INFO_UNION _machineInfo;//1024
        USERS_INFO_UNION _userInfo[MAX_SUPPORTED_USERS];//128*5
        ACCUMULATED_DATA_UNION _accumulatedInfo;//32
        CUSTOM_CONFIG_UNION _customWorkouts[MAX_SUPPORTED_CUSTOM_PRGS];//256*5
        MYFIRST5K_CONFIG_UNION _myfirst5kInfo[MAX_SUPPORTED_USERS];//16*5
    }fields;
//    unsigned char g_flashBuffer[3072];
}FLASH_DATA;
...
//WWW static void Default_Data(void);
static void Default_Data(bool isForcedToDefault);//WWW
...
static UINT8 SystemSaveStructedData(UINT8* pDest, UINT8 *pSrc, UINT16 dataLen,  UINT8 enableINT )
{
    ...
    UINT16 endPos = PAGE_SIZE/dataLen - 1;//2016/5/30,Eric
    UINT16 index = 0;
    for( index = 0; index < endPos; index++)
    {
...
#define SAVE_MYFIRST5K_PROGRAM_TO_FLASH_AND_DISABLEINT \
    HAL_FLASH_Burst_Write(FLASH_MYFIRST5K_INFO_ADDRESS, (UINT8 *)(&FLASH_DATA.fields._myfirst5kInfo[0]), sizeof(MYFIRST5K_CONFIG_UNION)*MAX_SUPPORTED_USERS ,0);
#define SAVE_MYFIRST5K_PROGRAM_TO_FLASH_AND_ENABLEINT \
    HAL_FLASH_Burst_Write(FLASH_MYFIRST5K_INFO_ADDRESS, (UINT8 *)(&FLASH_DATA.fields._myfirst5kInfo[0]), sizeof(MYFIRST5K_CONFIG_UNION)*MAX_SUPPORTED_USERS ,1);	


#define SAVE_CUSTOM_PROGRAM_TO_FLASH_AND_DISABLEINT \
    HAL_FLASH_Burst_Write(FLASH_CUSTOMTIME_PRG_ADDRESS, (UINT8 *)(&FLASH_DATA.fields._customWorkouts[0]), sizeof(CUSTOM_CONFIG_UNION)*MAX_SUPPORTED_USERS ,0);
#define SAVE_CUSTOM_PROGRAM_TO_FLASH_AND_ENABLEINT \
    HAL_FLASH_Burst_Write(FLASH_CUSTOMTIME_PRG_ADDRESS, (UINT8 *)(&FLASH_DATA.fields._customWorkouts[0]), sizeof(CUSTOM_CONFIG_UNION)*MAX_SUPPORTED_USERS ,1);	
...
void SystemConfig_Initial(void)
{
    ...
    if(FLASH_DATA.fields._machineInfo.mfields.firstBootFlag != 0xA5)
    {
        ...
        HAL_FLASH_Erase_Page(FLASH_CUSTOMTIME_PRG_ADDRESS,0);
        HAL_FLASH_Erase_Page(FLASH_MYFIRST5K_INFO_ADDRESS,0);
...
//WWW		Default_Data();
        ...
        SAVE_MYFIRST5K_PROGRAM_TO_FLASH_AND_DISABLEINT
    }
    else
    {
        ...
        HAL_FLASH_Burst_Read(FLASH_CUSTOMTIME_PRG_ADDRESS, FLASH_DATA.fields._customWorkouts[0].customProgramBuffer, (sizeof(CUSTOM_CONFIG_UNION))* MAX_SUPPORTED_USERS);
        
        //Step 5, read my first 5k data into _myfirst5kInfo
        HAL_FLASH_Burst_Read(FLASH_MYFIRST5K_INFO_ADDRESS,FLASH_DATA.fields._myfirst5kInfo[0].myfirst5kProgramBuffer,(sizeof(MYFIRST5K_CONFIG_UNION))* MAX_SUPPORTED_USERS);
    }
}
...
UCHAR SystemConfig_Set_MyFirst5kInfo(MYFIRST5K_CONFIG myfirst5kinfo , UCHAR userindex)
{
    if((userindex > MAX_SUPPORTED_USERS)||(0 == userindex))
        return 0;
    
    int result = memcmp(&myfirst5kinfo,&FLASH_DATA.fields._myfirst5kInfo[userindex -1].myfirst5kProgramBuffer,sizeof(MYFIRST5K_CONFIG));
    UCHAR ret = 0;
    if(result != 0)
    {
        memcpy(&FLASH_DATA.fields._myfirst5kInfo[userindex -1].myfirst5kProgramBuffer,&myfirst5kinfo,sizeof(MYFIRST5K_CONFIG));
        ret = 1;
    }
...
MYFIRST5K_CONFIG SystemConfig_Get_MyFirst5kInfo(UCHAR UserIndex)
{
    MYFIRST5K_CONFIG myfirst5kinforet;
    memset( &myfirst5kinforet, 0, sizeof(myfirst5kinforet));
    if(UserIndex > MAX_SUPPORTED_USERS) return myfirst5kinforet;
    
    if(UserIndex == 0)
    {
        myfirst5kinforet.Week = 1;
        myfirst5kinforet.Workout = 1;
        myfirst5kinforet.Walk_Speed = 20;
        myfirst5kinforet.Jog_Speed = 35;
        myfirst5kinforet.Reset_Counts = 0;
        myfirst5kinforet.lastUpdateYear = 2016;
        myfirst5kinforet.lastUpdateMonth = 1;
        myfirst5kinforet.lastUpdateDay = 1;
    }
    else
    {
        memcpy(&myfirst5kinforet, &FLASH_DATA.fields._myfirst5kInfo[UserIndex-1].myfirst5kProgramBuffer, sizeof(MYFIRST5K_CONFIG));
    }
    
    return myfirst5kinforet;
}

UCHAR SystemConfig_Set_CustomInfo(CUSTOM_CONFIG custominfo , UCHAR userindex)
{
    if((userindex > MAX_SUPPORTED_USERS)||(0 == userindex))
        return 0;
    
    int result = memcmp(&custominfo,&FLASH_DATA.fields._customWorkouts[userindex -1].customProgramBuffer,sizeof(CUSTOM_CONFIG));
    UCHAR ret = 0;
    if(result != 0)
    {
        memcpy(&FLASH_DATA.fields._customWorkouts[userindex -1].customProgramBuffer,&custominfo,sizeof(CUSTOM_CONFIG));
        ret = 1; 
    }
...
CUSTOM_CONFIG SystemConfig_Get_CustomInfo(UCHAR UserIndex)//,UCHAR Customindex)
{
    CUSTOM_CONFIG custominforet; 

    memset(&custominforet,0,sizeof(custominforet));
    if(UserIndex > MAX_SUPPORTED_USERS) return custominforet;
    if(UserIndex == 0)
    {
        custominforet.Custom_Time.customWorkoutTotalTime = 60;
        custominforet.Custom_Time.customSegmentNumber = 1;
        custominforet.Custom_Distance.customSegmentNumber = 1;

        for(char i = 0;i < MAX_CUSTOM_SEGMENTS;i++)
        {
            custominforet.Custom_Time.customWorkoutTime[i] = D_CUSTOM_SEGMENT_TIME;
            custominforet.Custom_Time.customSpeedX10[i] = D_CUSTOM_SPEED_MPH;
            custominforet.Custom_Time.customInclineX10[i] = D_CUSTOM_INCLINE;
            custominforet.Custom_Time.customresistance[i] = D_CUSTOM_RESISTANCE;
            
            custominforet.Custom_Distance.customWorkoutDistance[i] = D_CUSTOM_SEGMENT_MINDISTANCE;
            custominforet.Custom_Distance.customSpeedX10[i] = D_CUSTOM_SPEED_MPH;
            custominforet.Custom_Distance.customInclineX10[i] = D_CUSTOM_INCLINE;

            custominforet.Custom_Distance.customresistance[i] = D_CUSTOM_RESISTANCE;
        }
        
        for(char i = 0;i < MAX_CUSTOM_HR_SEGMENTS;i++)
        {
            custominforet.Custom_HR.customTargetHeartRate[i] = D_CUSTOM_TARGET_HR;
            custominforet.Custom_HR.customHRTime[i] = D_CUSTOM_HR_SEGMENT_TIME;
            custominforet.Custom_HR.customHRTotalTime = D_CUSTOM_HR_TOTAL_TIME;
        }
    }
    else
    {
        memcpy(&custominforet, &FLASH_DATA.fields._customWorkouts[UserIndex-1].customProgramBuffer, sizeof(CUSTOM_CONFIG));
    }
    return custominforet;
}
/***************************************************************************//**
* @brief
*    用于WIFI设置UserInfo
...
UCHAR SystemConfig_Get_Display_Unit(UCHAR userSlot)
{
	UCHAR ret = FLASH_DATA.fields._machineInfo.mfields.unit;
	if((userSlot>=1)&&(userSlot<=MAX_SUPPORTED_USERS))
	{
		ret = FLASH_DATA.fields._userInfo[userSlot-1].ufields.displayUnit;
	}
...
UINT16 SystemConfig_Get_Int(SYSTEM_CONFIG_TYPE type)
{
	...
	switch(type)
	{
		...
		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Time.customWorkoutTotalTime;
		break;
        case CUSTOM2_WORKOUT_MIN_UINT16:
		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Time.customWorkoutTotalTime;
		break;
        case CUSTOM3_WOKROUT_MIN_UINT16:
		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Time.customWorkoutTotalTime;
		break;
        case CUSTOM4_WORKOUT_MIN_UINT16:
		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Time.customWorkoutTotalTime;
		break;
        case CUSTOM5_WORKOUT_MIN_UINT16:
		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Time.customWorkoutTotalTime;
		break;
        
        //      case CUSTOM1_WORKOUT_MIN_UINT16:
        //		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Time.customWorkoutTime;
        //		break;
        //	  case CUSTOM2_WORKOUT_MIN_UINT16:
        //		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Time.customWorkoutTime;
        //		break;
        //	  case CUSTOM3_WOKROUT_MIN_UINT16:
        //		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Time.customWorkoutTime;
        //		break;
        //	  case CUSTOM4_WORKOUT_MIN_UINT16:
        //		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Time.customWorkoutTime;
        //		break;
        //      case CUSTOM5_WORKOUT_MIN_UINT16:
        //		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Time.customWorkoutTime;
        //		break;
...
UCHAR* SystemConfig_Get_String(SYSTEM_CONFIG_TYPE type)
{
	...
	switch(type)
	{
		...
		by_Dat=(UCHAR*)FLASH_DATA.fields._machineInfo.mfields.consoleSerialNumber;
		break;
        case FRAME_SERIAL_NUMBER_ARRAY:
		by_Dat=(UCHAR*)FLASH_DATA.fields._machineInfo.mfields.frameSerialNumber;
		break;
        case MARIO_BOX_ADDRESS_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._machineInfo.mfields.marioBox;
		break;
        case MACHINE_UUIND_ARRAY:
		by_Dat=(UCHAR*)FLASH_DATA.fields._machineInfo.mfields.machineUUID;
		break;
        case RTC_TIME_ZONE_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._machineInfo.mfields.timezone;
		break;
        //TIME
//        case CUSTOM1_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Time.customSpeedX10;
//		break;
//        case CUSTOM2_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Time.customSpeedX10;
//		break;
//        case CUSTOM3_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Time.customSpeedX10;
//		break;
//        case CUSTOM4_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Time.customSpeedX10;
//		break;
//        case CUSTOM5_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Time.customSpeedX10;
//		break;
        case CUSTOM1_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Time.customInclineX10;
		break;
        case CUSTOM2_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Time.customInclineX10;
		break;
        case CUSTOM3_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Time.customInclineX10;
		break;
        case CUSTOM4_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Time.customInclineX10;
		break;
        case CUSTOM5_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Time.customInclineX10;
		break;
        case CUSTOM1_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Time.customresistance;
		break;
        case CUSTOM2_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Time.customresistance;
		break;
        case CUSTOM3_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Time.customresistance;
		break;
        case CUSTOM4_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Time.customresistance;
		break;
        case CUSTOM5_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Time.customresistance;
		break;
        //DISTANCE 
//        case CUSTOM1_DISTANCE_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Distance.customSpeedX10;
//		break;
//        case CUSTOM2_DISTANCE_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Distance.customSpeedX10;
//		break;
//        case CUSTOM3_DISTANCE_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Distance.customSpeedX10;
//		break;
//        case CUSTOM4_DISTANCE_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Distance.customSpeedX10;
//		break;
//        case CUSTOM5_DISTANCE_SEGMENTS_ARRAY_SPEEDX10:
//		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Distance.customSpeedX10;
//		break;
        case CUSTOM1_DISTANCE_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Distance.customInclineX10;
		break;
        case CUSTOM2_DISTANCE_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Distance.customInclineX10;
		break;
        case CUSTOM3_DISTANCE_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Distance.customInclineX10;
		break;
        case CUSTOM4_DISTANCE_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Distance.customInclineX10;
		break;
        case CUSTOM5_DISTANCE_SEGMENTS_ARRAY_INCLINEX10:
		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Distance.customInclineX10;
		break;
        case CUSTOM1_DISTANCE_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[0].ccfields.Custom_Distance.customresistance;
		break;
        case CUSTOM2_DISTANCE_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[1].ccfields.Custom_Distance.customresistance;
		break;
        case CUSTOM3_DISTANCE_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[2].ccfields.Custom_Distance.customresistance;
		break;
        case CUSTOM4_DISTANCE_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[3].ccfields.Custom_Distance.customresistance;
		break;
        case CUSTOM5_DISTANCE_SEGMENTS_ARRAY_RESISTANCE:
		by_Dat=FLASH_DATA.fields._customWorkouts[4].ccfields.Custom_Distance.customresistance;
		break;
        //HR
        
        case USER1_INFO_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._userInfo[0].ufields.name;
		break;
        case USER2_INFO_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._userInfo[1].ufields.name;
		break;
        case USER3_INFO_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._userInfo[2].ufields.name;
		break;
        case USER4_INFO_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._userInfo[3].ufields.name;
		break;
        case USER5_INFO_STRING:
		by_Dat=(UCHAR*)FLASH_DATA.fields._userInfo[4].ufields.name;
		break;
...
//		memcpy(&(FLASH_DATA.fields._machineInfo.mfields.minInclineADC),p,sizeof(UINT16));
        ...
        case SVK_ADD_ACCUMULATED_MILESX100:
        {
            ...
            if((*(UINT32 *)p) == 0) 
                ...
                FLASH_DATA.fields._accumulatedInfo.accfields.accumulatedDistanceMilesX100 = *(UINT32*)p;
//                memcpy(&(FLASH_DATA.fields._accumulatedInfo.accfields.accumulatedDistanceMilesX100),p,sizeof(UINT32));
        ...
        case SVK_ADD_ACCUMULATED_SECONDS:
		{
            ...
            if((*(UINT32 *)p) == 0) 
                ...
                FLASH_DATA.fields._accumulatedInfo.accfields.accumulatedTimeSeconds = *(UINT32*)p;
//                memcpy(&(FLASH_DATA.fields._accumulatedInfo.accfields.accumulatedTimeSeconds),p,sizeof(UINT32));
        ...
        case SVK_ADD_ACCUMULATED_LUBE_MILESX100:
		{
            ...
            if( (*(UINT32 *)p) == 0) 
                ...
                FLASH_DATA.fields._accumulatedInfo.accfields.accumulatedLubeMilesX100 = *(UINT32*)p;
//                memcpy(&(FLASH_DATA.fields._accumulatedInfo.accfields.accumulatedLubeMilesX100),p,sizeof(UINT32));
        ...
        if(size != sizeof(UINT16)) return 0;
        FLASH_DATA.fields._machineInfo.mfields.erpOnWaitMinutes = *(UINT16*)p;
//		memcpy(&(FLASH_DATA.fields._machineInfo.mfields.erpOnWaitMinutes),p,sizeof(UINT16));
		break;
        case SVK_SET_DEMO_TIMES:
        if(size != sizeof(UINT16)) return 0;
        FLASH_DATA.fields._machineInfo.mfields.demoOnWaitMinutes = *(UINT16*)p;
//		memcpy(&FLASH_DATA.fields._machineInfo.mfields.demoOnWaitMinutes,p,sizeof(UINT16));
        ...
        case SVK_SAVE_ACCUMULATEDDATA2FLASH:
	 	if(SystemSaveStructedData((UINT8 *)(FLASH_ACCUMULATED_DATA_ADDRESS), (UINT8 *)(&FLASH_DATA.fields._accumulatedInfo),sizeof(ACCUMULATED_DATA_UNION), 1))
            ...
            ret = 0;
	 	break;
        case SVK_SAVE_MYFIRST5KPRG2FLASH:
        HAL_FLASH_Erase_Page(FLASH_MYFIRST5K_INFO_ADDRESS, 1);
	 	SAVE_MYFIRST5K_PROGRAM_TO_FLASH_AND_ENABLEINT
            ret = 1;
	 	break;
        case SVK_SAVE_CUSTOMPRG2FLASH:
	    HAL_FLASH_Erase_Page(FLASH_CUSTOMTIME_PRG_ADDRESS, 1);
...
void SystemConfigRestoreToFactory(void)
{
    ...
    if(machineInfo.mfields.firstBootFlag == 0xFF) return;
    if(HAL_FLASH_Erase_Page(FLASH_ACCUMULATED_DATA_ADDRESS, 1)  == false) return;
    if (HAL_FLASH_Erase_Page(FLASH_CUSTOMTIME_PRG_ADDRESS, 1) == false) return;
    if (HAL_FLASH_Erase_Page(FLASH_MACHINE_INFO_ADDRESS,1) == false) return;
...
char SystemConfig_Get_MaxCustomNumber(void)
{
	return(MAX_SUPPORTED_CUSTOM_PRGS);
}
/***************************************************************************//**
* @brief
...
//WWW void Default_Data(void)
void Default_Data(bool isForcedToDefault) //WWW
{
	MACHINE_INFO_UNION machineInfo;
    machineInfo.mfields = SystemConfig_Get_MachineInfo();
	//-------------------------------------------Accumulated Data--------------------------------------------
	...
	if(machineInfo.mfields.firstBootFlag == 0xFF)
	{
	...
	{
		FLASH_DATA.fields._machineInfo.mfields.unit=machineInfo.mfields.unit;
		FLASH_DATA.fields._machineInfo.mfields.isEnergySaverModeOn=machineInfo.mfields.isEnergySaverModeOn;
		FLASH_DATA.fields._machineInfo.mfields.IsMachineConfiged = machineInfo.mfields.IsMachineConfiged;
	...
	systemsettings = SystemSettingArray[SETTING_MARIOX_QA_ADDRESS];
	...
	for(UINT8 i = 0; i < MAX_SUPPORTED_USERS; i++ )
	{
        ...
        memset( FLASH_DATA.fields._userInfo[i].ufields.MMFtokenID, '0', 40 );
        memset( FLASH_DATA.fields._userInfo[i].ufields.MFPtokenID, '0', 320 );
        memset( FLASH_DATA.fields._userInfo[i].ufields.MFPuserID, '0', 15 );
        memset( FLASH_DATA.fields._userInfo[i].ufields.name, 0, 65 );
        sprintf(FLASH_DATA.fields._userInfo[i].ufields.name,"user %d",i+1);
		...
		FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryDataCount = 0;
       
        for(char j = 0;j < 10;j++)
        {
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workouttime = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutcalorie = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutdistance = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutmaxspeed = 120;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutaveragespeed = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutmaxheartrate = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutaverageheartrate = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutelevation = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutunits = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutstartyear = 2016;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutstartmonth = 1;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutstartday = 1;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutstarthour = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutstartminute = 0;
            FLASH_DATA.fields._userInfo[i].ufields.WorkoutSummaryData[j].workoutstartsecond = 0;
        }
    }
	//--------------------------------------Customized Programs-------------------------------------------------
	...
	{
        //Time
		FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Time.customWorkoutTotalTime = D_CUSTOM_TIME;
		FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Time.customSegmentNumber = 1;
		FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Distance.customSegmentNumber = 1;
        for(UINT8 j = 0; j < MAX_CUSTOM_SEGMENTS; j++)
		{
            //Time
            FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Time.customWorkoutTime[j] = D_CUSTOM_SEGMENT_TIME;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Time.customSpeedX10[j] = D_CUSTOM_SPEED_MPH;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Time.customInclineX10[j] = D_CUSTOM_INCLINE;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Time.customresistance[j] = D_CUSTOM_RESISTANCE;
		    //Distance
            FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Distance.customWorkoutDistance[j] = D_CUSTOM_SEGMENT_MINDISTANCE;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Distance.customSpeedX10[j] = D_CUSTOM_SPEED_MPH;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Distance.customInclineX10[j] = D_CUSTOM_INCLINE;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_Distance.customresistance[j] = D_CUSTOM_RESISTANCE;
        }
        //HR
        for(UINT8 j = 0; j < MAX_CUSTOM_HR_SEGMENTS; j++)
		{
            FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_HR.customTargetHeartRate[j] = D_CUSTOM_TARGET_HR;
			FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_HR.customHRTime[j] = D_CUSTOM_HR_SEGMENT_TIME;            
        }
        FLASH_DATA.fields._customWorkouts[i].ccfields.Custom_HR.customHRTotalTime = D_CUSTOM_HR_TOTAL_TIME;
	}
    //------------------------------------My First 5k Info-------------------------------------------
    for(UINT8 i = 0; i < MAX_SUPPORTED_USERS; i++ )
	{
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.Week = 1;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.Workout = 1;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.Walk_Speed = 20;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.Jog_Speed = 35;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.Reset_Counts = 0;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.lastUpdateYear = 2016;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.lastUpdateMonth = 1;
        FLASH_DATA.fields._myfirst5kInfo[i].MYfields.lastUpdateDay = 1;
    }
}

UINT32 SystemConfig_Get_FirstBoot_ADDRESS(void)
{
    UINT32 TempAdd= FLASH_MACHINE_INFO_ADDRESS + (sizeof(MACHINE_INFO) - 1);
    return(TempAdd);
}