...
void writeRFSettings(void)
{
    ...
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL1,  0x0B); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL0,  0x00); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ2,    0x0C); // Freq control word, high byte
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ1,    0x1D); // Freq control word, mid byte.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ0,    0x89); // Freq control word, low byte.
    ...
    CC_SPI_Write_Reg(TI_CCxxx0_FOCCFG,   0x1D); // Freq Offset Compense. Config
...
void writeRFSettings(void)
{
    ...
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL1,  0x0B); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL0,  0x00); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ2,    0x10); // Freq control word, high byte
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ1,    0xA7); // Freq control word, mid byte.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ0,    0x62); // Freq control word, low byte.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG4,  0x2D); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG3,  0x3B); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG2,  0x73); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG1,  0x22); // Modem configuration.
...
void writeRFSettings(void)
{
    ...
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL1,  0x0B); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL0,  0x00); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ2,    0x21); // Freq control word, high byte
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ1,    0x62); // Freq control word, mid byte.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ0,    0x76); // Freq control word, low byte.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG4,  0x2D); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG3,  0x3B); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG2,  0x73); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG1,  0x22); // Modem configuration.
...
void writeRFSettings(void)
{
    ...
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL1,  0x0B); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL0,  0x00); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ2,    0x23); // Freq control word, high byte
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ1,    0x31); // Freq control word, mid byte.
    CC_SPI_Write_Reg(TI_CCxxx0_FREQ0,    0x3B); // Freq control word, low byte.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG4,  0x2D); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG3,  0x3B); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG2,  0x73); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG1,  0x22); // Modem configuration.
...
{
    // Write register settings
    CC_SPI_Write_Reg(TI_CCxxx0_IOCFG2,   0x09);  // GDO2 output pin confi
    CC_SPI_Write_Reg(TI_CCxxx0_IOCFG0,   0x06);  // GDO0 output pin config.
    CC_SPI_Write_Reg(TI_CCxxx0_PKTLEN,   0xFF);  // Packet length.
    CC_SPI_Write_Reg(TI_CCxxx0_PKTCTRL1, 0x05);  // Packet automation control.
    CC_SPI_Write_Reg(TI_CCxxx0_PKTCTRL0, 0x05);  // Packet automation control.
    CC_SPI_Write_Reg(TI_CCxxx0_ADDR,     0x01);  // Device address.
    CC_SPI_Write_Reg(TI_CCxxx0_CHANNR,   0x00); // Channel number.
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL1,  0x0A); // Freq synthesizer control.
    CC_SPI_Write_Reg(TI_CCxxx0_FSCTRL0,  0x00); // Freq synthesizer control.
    ...
    CC_SPI_Write_Reg(TI_CCxxx0_MDMCFG0,  0xF8); // Modem configuration.
    CC_SPI_Write_Reg(TI_CCxxx0_DEVIATN,  0x00); // Modem dev (when FSK mod en)
    CC_SPI_Write_Reg(TI_CCxxx0_MCSM1 ,   0x0F); //MainRadio Cntrl State Machine
    CC_SPI_Write_Reg(TI_CCxxx0_MCSM0 ,   0x18); //MainRadio Cntrl State Machine
    CC_SPI_Write_Reg(TI_CCxxx0_FOCCFG,   0x1D); // Freq Offset Compens. Config
    CC_SPI_Write_Reg(TI_CCxxx0_BSCFG,    0x1C); //  Bit synchronization config.
    CC_SPI_Write_Reg(TI_CCxxx0_AGCCTRL2, 0xC7); // AGC control.
...
void RFSendPacket(char *txBuffer, char size)
{
    ...
    *(txBuffer+1) &= 0x7f; // send BIT7 is "0"
    CC_SPI_Write_Burst_Reg(TI_CCxxx0_TXFIFO, txBuffer, size); // Write TX data

    //HAL_RF_Config_GDO0_INT_Mode(0);
    HAL_RF_Config_GDO0(0);
    
    CC_SPI_Strobe(TI_CCxxx0_STX);         // Change state to TX, initiating
    // data transfer
    while(!HAL_RF_Get_GDO0_Pin_Input_State() && w_Count--);
    w_Count = ERROR_BREAK_TIME;
    while(HAL_RF_Get_GDO0_Pin_Input_State() && w_Count--);

    HAL_RF_Clear_INT_Flag(0);
    
   // HAL_RF_Config_GDO0_INT_Mode(1);
...
char RFReceivePacket(char *rxBuffer, char *length)
{
    ...
    if ((CC_SPI_Read_Status(TI_CCxxx0_RXBYTES) & TI_CCxxx0_NUM_RXBYTES))
    {
        ...
        {
            CC_SPI_Read_Burst_Reg(TI_CCxxx0_RXFIFO, rxBuffer, pktLen); // Pull data
            *length = pktLen;                     // Return the actual size
            CC_SPI_Read_Burst_Reg(TI_CCxxx0_RXFIFO, status, 2);
