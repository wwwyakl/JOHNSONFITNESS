...
static const RX_DATA_PROPERTY RxDataOffset[RX_DATA_LEN] =
{
    ...
    { DG_NEW_VERSION,       0 },     //READ NEW MCB VERSION
...
#define LCB_SPEED_TIMER     800
#define LCB_RPM_MAX         5000
#define LCB_RPM_ADD_STEP    110
#define LCB_RPM_DEC_STEP    130
...
#define C_KEEP_ONLINE_IDLE  300         //ms
...
void LCB_Initial(void)
{
    ...
    for(UCHAR i=0; i<RX_DATA_LEN; i++)
    {
        ...
        {
            Offset_Adress[i].RxLen = RxDataOffset[i].RxLen;;
        }
...
void LCB_Speed_Update(void)
{
    ...
    {
        if((w_CurrentSpeed - SpeedRef.DecStep) >= wTargetSpeed)
        {
