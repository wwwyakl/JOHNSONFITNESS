...
unsigned short ADC_channel=0;//RESISTANCE 
unsigned short ADC_started=0;
/** Internal Functions */
static UCHAR HAL_Get_IO(GPIO_Port_TypeDef by_Port, UINT w_Pin);

static void HAL_Enable_IO_Int(UINT w_Pin);
//static void HAL_Set_IO_IntMode(GPIO_Port_TypeDef by_Port, UINT w_Pin, UCHAR by_Mode,UCHAR by_Enable);
...
extern void HT1621_Initial_Command(char by_IC);
...
static void HAL_WDT_Initial_IO(void)
{
    ...
    WDOG_Init_TypeDef init =
    {
        ...
        .clkSel     = wdogClkSelULFRCO,     /* Select Ultra low frequency (1 kHz) clock */
...
void HAL_Key_Initial_IO(PHYKEYMODE mode)
{
    ...
    if(mode==KEYMODE_IO)
    {
        ...
        Hal_Regist_ISR(MS_ISR,HAL_KeyMatrix_1ms,HAL_Key_Matrix_1msScan_Int);    
    }
    else
    {
...
#endif        
        /* rigist I2C ODDINT  */   
        HAL_INT_Regist_Pin_INT(PIN_INT_I2C,HAL_Key_I2C_IntISR); 

		I2C_Key_First = 1;		
        //初始化时读出key IC的值，避免以后读不出来 
       /* i2c_data.addr = 0xB0;
        i2c_data.flags = I2C_FLAG_WRITE_READ;
        i2c_data.buf[0].data = &trans_data[0];
        i2c_data.buf[0].len = 1;
...
#define PWM_TIMER_LOCATION          TIMER_ROUTE_LOCATION_LOC1
...
void HAL_PWM_Initial(PWM_CC_CHANNEL channel )
{
  ...
  switch(channel)
  {
      ...
      case CHANNEL_FAN: 
        ...
        TIMER_CompareBufSet(TIMER2, 1, 26666);
        break;
      case CHANNEL_AMP:
...
* @param[out] none 
******************************************************************************/
void HAL_Capture_Initial_IO()	//TIM1 CC1_Capture
{	
...
//	{
//		.eventCtrl  = timerEventEveryEdge,
//		.edge       = timerEdgeRising,//,timerEdgeRising
//		.prsSel     = timerPRSSELCh0,
//		.cufoa      = timerOutputActionNone,
//		.cofoa      = timerOutputActionNone,
...
//    
//	TIMER1->ROUTE = TIMER_ROUTE_CC1PEN | TIMER_ROUTE_LOCATION_LOC2 | TIMER_ROUTE_CC2PEN;
//	
//    /* Select timer parameters */  
//    TIMER_Init_TypeDef timerInit =
//    {
...
//    TIMER_IntEnable(TIMER1, TIMER_IF_CC2);// TIMER_IF_OF | 
...
#define DAC_TIMER    TIMER0
#define TIMER_CLOCK    cmuClock_TIMER0

#define PRS_CH_CTRL_SOURCESEL    PRS_CH_CTRL_SOURCESEL_TIMER0
#define PRS_CH_CTRL_SIGSEL_TIMEROF PRS_CH_CTRL_SIGSEL_TIMER0OF
...
#define PWM_DUTY 5
#define PWM_FREQUENCE 2000
/******************************************************************************/
void HAL_Beep_Initial_IO(void)
{
    GPIO_PinModeSet(BEEP_PORT, BEEP_PIN, gpioModePushPull, 0);
    
    LETIMER_CompareSet(BEEP_TIMER, BEEP_TIMER_COMP0, CMU_ClockFreqGet(cmuClock_LETIMER0)/PWM_FREQUENCE);
    LETIMER_CompareSet(BEEP_TIMER, BEEP_TIMER_COMP1, (CMU_ClockFreqGet(cmuClock_LETIMER0)/PWM_FREQUENCE)*PWM_DUTY/100);
...
//	else
//	{
//		/* Disable DAC channel 0, located on pin PB11 */
...
//    HAL_AMP_Set_Duty(DutyValue);
    ...
    //HAL_Set_IO_Output(Audio_Source3_PORT, Audio_Source3_PIN);
    HAL_Set_IO_Output(Volume_PWM_PORT, Volume_PWM_PIN);
	//HAL_Set_IO_High(Volume_PWM_PORT, Volume_PWM_PIN);
    HAL_Set_IO_Output(Volume_MUTE_PORT, Volume_MUTE_PIN);
...
{
	HAL_Set_IO_Output(TLC59282_DAT_PORT,TLC59282_DAT_PIN);
	HAL_Set_IO_Output(TLC59282_CLK_PORT,TLC59282_CLK_PIN);
	HAL_Set_IO_Output(TLC59282_OE_PORT,TLC59282_OE_PIN);	
	HAL_Set_IO_High(TLC59282_OE_PORT,TLC59282_OE_PIN);	
	HAL_Set_IO_Output(TLC59282_LE_PORT,TLC59282_LE_PIN);
...
{
	HAL_Set_IO_Output(HT1650_DATA_PORT,HT1650_DATA_PIN);       // dat   set output ****
	HAL_Set_IO_Output(HT1650_CLK_PORT,HT1650_CLK_PIN);	   // clock
	HAL_Set_IO_Output(HT1650_CS1_PORT,HT1650_CS1_PIN);	   // cs0
	//HAL_Set_IO_Output(HT1650_CS2_PORT,HT1650_CS2_PIN);	   //cs1
	
	HT1621_Initial_Command(0);
	HT1621_Initial_Command(1);
	HT1621_Initial_Command(2);
}
/***************************************************************************//**
* @brief
*   设置WR为高电平或者低电平   
...
* @param[out]：none   
******************************************************************************/
void HW_HT162x_Set_High_WR(UCHAR by_Dat)
{
#ifdef TM
    {
        if(by_Dat != 0)
        {
            //    HAL_Set_IO_Low(HT1650_CLK_PORT, HT1650_CLK_PIN);
            HAL_Set_IO_Low(HT1650_CLK_PORT, HT1650_CLK_PIN);
        }
        else
        {
            HAL_Set_IO_High(HT1650_CLK_PORT, HT1650_CLK_PIN);
            //    HAL_Set_IO_High(HT1650_CLK_PORT, HT1650_CLK_PIN);
        }   
    }
#else
...
* @param[out]：none   
******************************************************************************/
void HW_HT162x_Set_High_Data(UCHAR by_Dat)
{
    #ifdef TM
    {
        if(by_Dat != 0)
        {
            HAL_Set_IO_Low(HT1650_DATA_PORT,HT1650_DATA_PIN);
        }
        else
        {
            HAL_Set_IO_High(HT1650_DATA_PORT,HT1650_DATA_PIN);
        }
    }
#else
    {
...
* @param[out]：none 
******************************************************************************/
void HW_HT162x_Set_High_CS(UCHAR by_IC, UCHAR by_Dat)
{
    switch(by_IC)
    {
        case 0:
        {
#ifdef TM
            {
                if(by_Dat != 0)
                {
                    HAL_Set_IO_Low(HT1650_CS1_PORT, HT1650_CS1_PIN);
                }
                else
                {
                    HAL_Set_IO_High(HT1650_CS1_PORT, HT1650_CS1_PIN);
                }
            }
#else
            {
...
//			if(by_Dat != 0)
//            {
//				HAL_Set_IO_Low(HT1650_CS2_PORT, HT1650_CS2_PIN);
//            }
//            else
//            {
//				HAL_Set_IO_High(HT1650_CS2_PORT, HT1650_CS2_PIN);
//            }
//            break;
//        }
		//  case 2:
		...
		//      if(by_Dat != 0)
		//      {
		//        HW_Set_IO_Low(HT162X_CS2_PORT, HT162X_CS2_PIN);		//add by ckm 20110906
		//      }
		//      else
		//      {
		//        HW_Set_IO_High(HT162X_CS2_PORT, HT162X_CS2_PIN);
		//      }
		//      break;
		//    }
        default:break;
...
{
    HAL_Set_IO_Input(Switch_A_PORT,Switch_A_PIN);
    HAL_Set_IO_Input(Switch_B_PORT,Switch_B_PIN);
}

bool HAL_SwitchA_Pin_Input_State(void)
{
    return (HAL_Get_IO(Switch_A_PORT, Switch_A_PIN));
}

bool HAL_SwitchB_Pin_Input_State(void)
{
    return (HAL_Get_IO(Switch_B_PORT, Switch_B_PIN));
}
/***************************************************************************//**
* @brief
...
static volatile UINT16 Reset_From_ERP_Flag = 0 ;
/***************************************************************************//**
* @brief
*    HAL_Time_Delay
*   简单软件延时
...
* @note
*   LETIMER￡?
*   è?￡oHal_ERP_Regist_LETIMER_ISR(ERP_TIMER_COUNT,LETIMER_Count);
*
* @param[in]￡oLETIMER_IDX index, PF pf
* @param[out]￡onone 
******************************************************************************/
PF pfLETIMER_INTISR[ALL_LETIMERIDXS];

void HAL_ERP_Regist_LETIMER_ISR(LETIMER_IDX index, PF pf)
{
    if(pf == NULL) return;
    
    if( index < ALL_LETIMERIDXS)
    {
        pfLETIMER_INTISR[index] = pf;
    }
}
/***************************************************************************//**
* @brief
...
#ifdef TM    
    if(ulState & (0x0001 << UART_RX_PIN)||ulState & (0x0001 << SAFETYKEY_CHECK_PIN))
#else
...
* @param[out]:none
******************************************************************************/
static void HAL_ERP_LETIMER0_IRQHandler(void)
{
#ifndef DEBUG
    HAL_Clear_WatchDog();
#endif
    ...
    for(UINT8 i = 0; i < ALL_LETIMERIDXS; i++)
    {
        if(pfLETIMER_INTISR[i] != NULL)
            (*pfLETIMER_INTISR[i])();
    }
}
...
{
  //ERP LED
  GPIO_PinModeSet(ERP_LED_PORT, ERP_LED_PIN, gpioModeInput, 0);
  //Heart&Polar Port
  ...
  //BT
  GPIO_PinModeSet(BT_UART_TX_PORT, BT_UART_TX_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(BT_UART_RX_PORT, BT_UART_RX_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(BT_RST_PORT, BT_RST_PIN, gpioModeInput, 0);
  NVIC_DisableIRQ(DIGITAL_BT_UART_Tx_IRQ);
  NVIC_DisableIRQ(DIGITAL_BT_UART_Rx_IRQ);
  TV_PORT_UART->ROUTE |= ~DIGITAL_BT_UART_ROUTE_EN; 
  
  GPIO_PinModeSet(BlueTooth_TX_PORT, BlueTooth_TX_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(BlueTooth_RX_PORT, BlueTooth_RX_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(BlueTooth_RST_PORT, BlueTooth_RST_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(BlueTooth_WAKEUP_PORT, BlueTooth_WAKEUP_PIN, gpioModeInput, 0);
  NVIC_DisableIRQ(BLUETOOTH_UART_Rx_IRQ);
  NVIC_DisableIRQ(BLUETOOTH_UART_Tx_IRQ);
  TV_PORT_UART->ROUTE |= ~BLUETOOTH_UART_ROUTE_EN; 

  
  //EEPROM
  GPIO_PinModeSet(EEPROM_SK_PORT, EEPROM_SK_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(EEPROM_DI_PORT, EEPROM_DI_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(EEPROM_D0_PORT, EEPROM_D0_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(EEPROM_CS_PORT, EEPROM_CS_PIN, gpioModeInput, 0);

  //RTC
  GPIO_PinModeSet(HT1381_CLK_PORT, HT1381_CLK_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(HT1381_DAT_PORT, HT1381_DAT_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(HT1381_RST_PORT, HT1381_RST_PIN, gpioModeInput, 0);
  //AMP
  ...
  //DISPLAY LCD
//  GPIO_PinModeSet(HT16XX_DAT0_PORT, HT16XX_DAT0_PIN, gpioModeInput, 0);
//  GPIO_PinModeSet(HT16XX_CLK_PORT, HT16XX_CLK_PIN, gpioModeInput, 0);
...
//  GPIO_PinModeSet(HT16XX_CS0_PORT, HT16XX_CS0_PIN, gpioModeInput, 0);
//  GPIO_PinModeSet(LCD_BACKLIGHT_PORT, LCD_BACKLIGHT_PIN, gpioModeInput, 0);
  //DISPLAY LED
  GPIO_PinModeSet(TLC59282_DAT_PORT, TLC59282_DAT_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(TLC59282_CLK_PORT, TLC59282_CLK_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(TLC59282_OE_PORT, TLC59282_OE_PIN, gpioModeInput, 0);
  GPIO_PinModeSet(TLC59282_LE_PORT, TLC59282_LE_PIN, gpioModeInput, 0);
...
{  
    ...
    NVIC_ClearPendingIRQ(BLUETOOTH_UART_Rx_IRQ);
    NVIC_ClearPendingIRQ(BLUETOOTH_UART_Tx_IRQ);
    NVIC_ClearPendingIRQ(DIGITAL_BT_UART_Rx_IRQ);
    NVIC_ClearPendingIRQ(DIGITAL_BT_UART_Tx_IRQ);
    NVIC_ClearPendingIRQ(ADC0_IRQn);
    NVIC_DisableIRQ(TIMER1_IRQn);
    NVIC_ClearPendingIRQ(TIMER1_IRQn);
    ADC_IntClear(ADC0, _ADC_IF_MASK);
    USART_IntClear(DIGITAL_WIFI_UART, _USART_IF_MASK);
    USART_IntClear(BLUETOOTH_UART, _USART_IF_MASK);
    USART_IntClear(DIGITAL_BT_UART, _USART_IF_MASK);
    HAL_Time_Delay(50000);
...
void Hal_Regist_ISR(INTERRUPT_ISR isr, MSISR_IDX index, PF pf)
{
    ...
    if(isr ==  MS_ISR && index < ALL_IDXS)
    {
        pfMsISR[index] = pf;
    }
    else if(isr !=  MS_ISR && isr < ALL_ISRS)
    {
        pfISR[isr] = pf;
    }
}
...
{
    for(UINT8 i = 0; i < ALL_INTIDXS; i++)
    {
        if(pf_INT_ISR[i] != NULL)
            (*pf_INT_ISR[i])();
    }
}
/***************************************************************************//**
...
{ 
    ...
    if(HAL_RPM_Get_EXIFlg())
    {
        HAL_RPM_Clear_EXIFlg();
        RPM_Reload_Int();
    }
#endif
}
...
#if (defined EP)||(defined AE) 
//---------------------------RPM function--------------------//
void HAL_RPM_Initial_IO(void)
{
	HAL_Set_IO_Input(RPM_PORT, RPM_PIN);
	HAL_Set_IO_IntMode(RPM_PORT, RPM_PIN, INT_FALLING_EDGE, true);
	HAL_Enable_IO_Int(RPM_PIN);
	NVIC_ClearPendingIRQ(GPIO_ODD_IRQn);
	NVIC_EnableIRQ(GPIO_ODD_IRQn);
}
...
{
	return(HAL_Get_IO_IntFlag(RPM_PIN));
}

void HAL_RPM_Clear_EXIFlg(void)
{
	HAL_Clear_IO_IntFlag(RPM_PIN);
}

//---------------------ECB   VR2 function--------------------//
void HAL_ECB_Initial_IO(void)
{
	HAL_Set_IO_Output(EMS_ECB_UP_PORT,  EMS_ECB_UP_PIN);
	HAL_Set_IO_Output(EMS_ECB_DOWN_PORT,EMS_ECB_DOWN_PIN);
//	HAL_Set_IO_Output(SVR1_PORT,SVR1_PIN);
              HAL_Set_IO_Input(SVR1_PORT,SVR1_PIN);
//	HAL_Set_IO_High(SVR1_PORT,SVR1_PIN);             
}

void HAL_ECB_UP(void)
{
	HAL_Set_IO_High(EMS_ECB_UP_PORT,EMS_ECB_UP_PIN);
	HAL_Set_IO_Low(EMS_ECB_DOWN_PORT,EMS_ECB_DOWN_PIN);
}

void HAL_ECB_DOWN(void)
{
	HAL_Set_IO_High(EMS_ECB_DOWN_PORT,EMS_ECB_DOWN_PIN);
	HAL_Set_IO_Low(EMS_ECB_UP_PORT,EMS_ECB_UP_PIN);
}

void HAL_ECB_NONE(void)
{
	HAL_Set_IO_Low(EMS_ECB_DOWN_PORT,EMS_ECB_DOWN_PIN);
	HAL_Set_IO_Low(EMS_ECB_UP_PORT,EMS_ECB_UP_PIN);
}

unsigned char HAL_ECB_Check_Up(void)
{
    return HAL_Get_IO(EMS_ECB_UP_PORT,EMS_ECB_UP_PIN);
}

unsigned char HAL_ECB_Check_Down(void)
{
    return HAL_Get_IO(EMS_ECB_DOWN_PORT,EMS_ECB_DOWN_PIN);
}

/*
void HAL_VR2_Initial_IO(void)
{
	//HW_Set_IO_Output(VR2_PORT, VR2_PIN);
	
	ADC_Init_TypeDef init = ADC_INIT_DEFAULT;
	ADC_InitSingle_TypeDef singleInit = ADC_INITSINGLE_DEFAULT;
	init.ovsRateSel = adcOvsRateSel32;
	init.timebase = ADC_TimebaseCalc(0);
	init.prescale = ADC_PrescaleCalc(VR2_ADC_SPEED, 0);
	ADC_Init(ADC0, &init);
	
//	singleInit.reference = VR2_ADC_REFERENCE;
              singleInit.reference = adcRef5VDIFF;  //	adcRefVDD adcRef5VDIFF
//	singleInit.input = VR2_ADC_Input;
              singleInit.input = adcSingleInpCh4;	
	singleInit.resolution = VR2_ADC_Resolution;
	singleInit.acqTime = adcAcqTime64;
	ADC_InitSingle(ADC0, &singleInit);
	
//	ADC_IntEnable(ADC0, 0x0001);
              ADC_IntEnable(ADC0, 0x0004);	
	NVIC_ClearPendingIRQ(ADC0_IRQn);
	NVIC_EnableIRQ(ADC0_IRQn);
	ADC_Start(ADC0, adcStartSingle);
}
*/
void HAL_ADC_SINGLE_Initial(void)
{
	//HW_Set_IO_Output(VR2_PORT, VR2_PIN);	
	ADC_Init_TypeDef init = ADC_INIT_DEFAULT;
	ADC_InitSingle_TypeDef singleInit = ADC_INITSINGLE_DEFAULT;
...
void HAL_RESISTANCE_VR_ADCInitial(void)
{	
//	singleInit.reference = VR2_ADC_REFERENCE;
              ADC_InitSingle_TypeDef singleInit = ADC_INITSINGLE_DEFAULT;
              singleInit.reference = adcRefVDD;  //	adcRefVDD adcRef5VDIFF
//	singleInit.input = VR2_ADC_Input;
              singleInit.input = adcSingleInpCh4;	
	singleInit.resolution = VR2_ADC_Resolution;
	singleInit.acqTime = adcAcqTime64;
...
#if defined ( AE)
void ADC_Switch_Incline(void)
{
        ADC_InitSingle_TypeDef singleInit = ADC_INITSINGLE_DEFAULT;
        singleInit.reference = adcRefVDD;//adcRef3VDIFF; //adcRefVDD  adcRef2V5
...
void HAL_INCLINE_VR_ADCInitial(void)
{
//	singleInit.reference = VR2_ADC_REFERENCE;
              ADC_InitSingle_TypeDef singleInit = ADC_INITSINGLE_DEFAULT;
              singleInit.reference = adcRefVDD;  //	adcRefVDD adcRef5VDIFF
//	singleInit.input = VR2_ADC_Input;
              singleInit.input = adcSingleInpCh7;//adcSingleInpCh7;	
	singleInit.resolution = INCLINE_VR_ADC_Resolution;
	singleInit.acqTime = adcAcqTime64;
	ADC_InitSingle(ADC0, &singleInit);	
              
              ADC_IntEnable(ADC0, 0x04);//0x07	
	NVIC_ClearPendingIRQ(ADC0_IRQn);
	NVIC_EnableIRQ(ADC0_IRQn);
}

void HAL_INCLINE_VR_Get_CrrntChk_IO(void)
{
	ADC_Start(ADC0, adcStartSingle); 
}
void HAL_Incline_Initial_IO(void)
{   
    HAL_Set_IO_Output(EMS_INCLINE_UP_PORT,  EMS_INCLINE_UP_PIN);
    HAL_Set_IO_Output(EMS_INCLINE_DOWN_PORT,EMS_INCLINE_DOWN_PIN);
    HAL_Set_IO_Input(EMS_INCLINE_VR_PORT,EMS_INCLINE_VR_PIN);
//    GPIO_PinModeSet(EMS_INCLINE_VR_PORT, EMS_INCLINE_VR_PIN, gpioModeInputPull, 0);
    
}

unsigned char HAL_Incline_Check_Up(void)
{
//    return (unsigned char)p3_0;
    return HAL_Get_IO(EMS_INCLINE_UP_PORT,EMS_INCLINE_UP_PIN);
}

unsigned char HAL_Incline_Check_Down(void)
{
//    return (unsigned char)p3_1;
    return HAL_Get_IO(EMS_INCLINE_DOWN_PORT,EMS_INCLINE_DOWN_PIN);
}

void HAL_Incline_Set_Up(unsigned char by_Enable)
{
    if(by_Enable)
    {
        HAL_Set_IO_High(EMS_INCLINE_UP_PORT,EMS_INCLINE_UP_PIN);
        HAL_Set_IO_Low(EMS_INCLINE_DOWN_PORT,EMS_INCLINE_DOWN_PIN);
    }
    else
    {
        HAL_Set_IO_Low(EMS_INCLINE_UP_PORT,EMS_INCLINE_UP_PIN);
        HAL_Set_IO_Low(EMS_INCLINE_DOWN_PORT,EMS_INCLINE_DOWN_PIN);
    }
}

void HAL_Incline_Set_Down(unsigned char by_Enable)
{
      if(by_Enable)
    {
        HAL_Set_IO_High(EMS_INCLINE_DOWN_PORT,EMS_INCLINE_DOWN_PIN);
        HAL_Set_IO_Low(EMS_INCLINE_UP_PORT,EMS_INCLINE_UP_PIN);
    }
    else
    {
...
{
    unsigned long state;
    state = USART_IntGet(UART0);
    /* Reset all pending interupts */
    USART_IntClear(UART0, ~_UART_IFC_RESETVALUE);
...
void ADC0_IRQHandler(void)
{
     ...
     HAL_Reset_System();
      ...
      ECB_VR2_ADC_IntHandle(ADC_DataSingleGet(ADC0));
#else   
        ADC_IntClear(ADC0, ADC_IF_SINGLE);
        ADC_started=0; 
        if(ADC_channel==0) //RESIS
        {
                ECB_VR2_ADC_IntHandle(ADC_DataSingleGet(ADC0));
                ADC_channel=1;//INCLINE
        }
        else
        {
                Incline_Proce_AE(ADC_DataSingleGet(ADC0));
                ADC_channel=0;//RESIS
        }     
        ADC0_1MS_Handler();
#endif	
...
void HAL_BT_Initial_IO(void)
{
    USART_InitAsync_TypeDef UART_init = USART_INITASYNC_DEFAULT;
    UART_init.baudrate = 115200;
    USART_InitAsync(DIGITAL_BT_UART, &UART_init);
    DIGITAL_BT_UART->IFC = _UART_IFC_MASK;
    NVIC_ClearPendingIRQ(DIGITAL_BT_UART_Rx_IRQ);
    NVIC_ClearPendingIRQ(DIGITAL_BT_UART_Tx_IRQ);
    DIGITAL_BT_UART->IEN = UART_IEN_RXDATAV;
    GPIO_PinModeSet(BT_UART_TX_PORT, BT_UART_TX_PIN, gpioModePushPull, 1); //tx
    GPIO_PinModeSet(BT_UART_RX_PORT, BT_UART_RX_PIN, gpioModeInput, 1); //rx    
    DIGITAL_BT_UART->ROUTE |= DIGITAL_BT_UART_ROUTE_EN | DIGITAL_BT_UART_ROUTE_LOCATION;    
    //DIGITAL_BT_UART->TRIGCTRL
    NVIC_EnableIRQ(DIGITAL_BT_UART_Rx_IRQ);
//    HAL_BT_Enable_UARTI(0);       
    //HAL_Set_IO_Output(BT_RST_PORT, BT_RST_PIN);
    HAL_Set_IO_Input(BT_RST_PORT, BT_RST_PIN); 
    //HAL_Set_IO_Input(BT_INT_PORT,BT_INT_PIN);
    //HAL_Set_IO_Input(BT_Ready_PORT,BT_Ready_PIN);
}
void HAL_BT_SendData(UCHAR Data)
{
    USART_Tx(DIGITAL_BT_UART, Data);
}
void HAL_BT_Enable_UARTI(bool isEnabled)
{
   UINT16 TimeOut = 0;
    if(isEnabled == 0)
    {
        while(!(DIGITAL_BT_UART->STATUS & UART_STATUS_TXC) && TimeOut < 10000)
        {
            TimeOut++;
        }
        NVIC_ClearPendingIRQ(DIGITAL_BT_UART_Tx_IRQ);
        NVIC_DisableIRQ(DIGITAL_BT_UART_Tx_IRQ);
        USART_IntClear(DIGITAL_BT_UART, UART_IEN_TXC);
        USART_IntDisable(DIGITAL_BT_UART, UART_IEN_TXC);
    }
    else
    {
        NVIC_EnableIRQ (DIGITAL_BT_UART_Tx_IRQ);
        USART_IntSet   (DIGITAL_BT_UART, UART_IFS_TXC);
        USART_IntEnable(DIGITAL_BT_UART, UART_IEN_TXC);
    }
}
