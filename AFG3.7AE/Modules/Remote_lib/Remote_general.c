...
//            data->modelType = machineConfig.machineType;
            ...
            {
                data->machineType = 0;  //  0,TREADMILL£»2 ,BIKE£»
                data->modelType = 3;
            }
            else
            {
#ifdef  AE
              {
                data->machineType = 1;  //  1,ELLIPTICAL;3,ASCENT;
                data->modelType = 1;  //  INCLUDE INCLINE£»
              }
#else
              {
                data->machineType = 1;  //  1,ELLIPTICAL;3,ASCENT;
                data->modelType = 0;  //  NOT INCLUDE INCLINE£»
              }
#endif
            }
...
//            addstatefunc(REMOTE_PROPERTY_CONTROL<<8|CONTROL_GET_NETWORK_TIME, 0, 0, NULL );
      ...
      #else       
	 //     addstatefunc(REMOTE_PROPERTY_GENERAL<<8|GENERAL_ACKNOWLEDGMENT, GENERAL_GET_MACHINE_STATE, SetUserInfo(reponse), NULL );     
              addstatefunc(REMOTE_PROPERTY_GENERAL<<8|GENERAL_ACKNOWLEDGMENT, GENERAL_SET_USER_DATA, SetUserInfo(reponse), NULL );
      #endif
        }
        break;
        case GENERAL_GET_USER_DATA:
