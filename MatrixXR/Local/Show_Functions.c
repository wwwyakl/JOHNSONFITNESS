...
unsigned char Led_String(unsigned char *String, unsigned char StringNumber, unsigned char Motion)
{
    ...
    {// 大于窗口可以显示的字数时，以转动的方式显示
        Motion = ((StringLength > StringWindow_NowLetters)?Show_DownToUp:Show_MiddleFixed);
...
unsigned char DotMatrix_TableSearch(unsigned char SearchLetter)
{
    ...
    if(SearchLetter == 132 ) by_Out = 'a' ;
    if(SearchLetter == 142 ) by_Out = 'A' ;
    if(SearchLetter == 148 ) by_Out = 'o' ;
    if(SearchLetter == 153 ) by_Out = 'O' ;
    if(SearchLetter == 154 ) by_Out = 'U' ;
    if(SearchLetter == 129 ) by_Out = 'u' ;
	if(SearchLetter >= ' ' && SearchLetter <= 'z') 
        by_Out = SearchLetter-' ';
    else
        by_Out = 0;
...
unsigned char DotMatrix_SetString(unsigned char *String, unsigned char Motion)
{
    ...
    unsigned char by_blank = 0;
	unsigned char by_blank2 = 0;
    ...
    else
    {
        if(DotMatrixIntervalTimer >= DotMatrix_DotTime_ || Show_State.Flag.DataUpdate == 1)
        {
            ...
            switch(Motion)
            {
				...
				if(DotMovedNumber_String == (Letter_Height + 3))
					Rtn = 2;
				else if(DotMovedNumber_String >= (Letter_Height + 2))//rtn先等于1后等于2，可以减少滚动切换时间
					Rtn = 1;
				break;
                case Show_RightToLeft:
            ...
            {       
                for(j = 0; j < Letter_Width; j++)
                {
                    switch(Motion)
                    {
                        ...
                        case Show_MiddleFixed:  
						{
							DotCheck = i*Letter_Width + j + Window_ShiftDots+by_blank-by_blank2;
							if(DotCheck < StringWindow_NowWidth)
							{
								...
								else if( *(String + i) == 'T' || *(String + i) == 'M' || *(String + i) == 'W' || *(String + i) == 't' || *(String + i) == 'm' || *(String + i) == 'w')
								{
									if(4 == j)
									{
										by_blank++;
                        ...
                        case Show_DownToUp:
						{
                            ...
                            for(char i = 0;i < Window_ShiftDots;i++)
...
{    
    ...
    if((SetTimeOut&(1<<GroupNumber)) != 0 || Motion == Show_Fixed)
    {
        ...
        switch(Motion)
        {
			...
			DotMovedNumber_Small[GroupNumber]++;
			if(DotMovedNumber_Small[GroupNumber] >= (Letter_Height + 1)) //(Letter_Height * 2 + 1))
				DotMovedNumber_Small[GroupNumber] = 0; //0;
			break;
            case Show_RightToLeft:
...
void Show_Timer1m_INT(void)
{
	Show_Timer();
}
void Show_Regist_Functions(void)
{
	HAL_SystemTicker_Alloc_ISR(Show_Timer1m_INT);
}
