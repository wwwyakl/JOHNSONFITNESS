...
static UINT32 HAL_Key_TCA8418_Get_Value(void);
static void HAL_Process_TCA8418_Value(UINT32 keyNumber);
...
__no_init unsigned char trans_data[11];
static I2C_TransferReturn_TypeDef trans_return_value;   
__no_init static  unsigned char I2cKeytype = 3;
...
const static unsigned char Tca8418_Table[10][8] = 
{
...
#define TCA8418  2
#define SN7326    1
#define UNKONW_TYPE 3
#define __TCA8418_DEBOUNCE__
...
#define CLEAR_INT_FLAG   0xFF

#define TCA8418_MAX_COLS   10

#define TCA8418_ADDRESS 0x68
#define SN7326_ADDRESS 0xB0

static UINT32 lastkey = 0;
static UINT32 keyi2c = 0; 
static UINT8 keycheckcnt = 0;

static UCHAR I2c0_read(UCHAR add,UCHAR reg,UCHAR *pOut)
{
    I2C_TransferReturn_TypeDef ret = i2cTransferNack;
    I2C_TransferSeq_TypeDef i2c_data;  
    i2c_data.addr = add;
    i2c_data.flags = I2C_FLAG_WRITE_READ;
    i2c_data.buf[0].data = &reg;
    i2c_data.buf[0].len = 1;
    i2c_data.buf[1].data = pOut;
    i2c_data.buf[1].len = 1;
    ret = I2C_TransferInit(I2C0,&i2c_data);  
...
static I2C_TransferReturn_TypeDef  I2c0_write(UCHAR add,UCHAR reg,UCHAR pIut)
{
    I2C_TransferReturn_TypeDef ret = i2cTransferNack;
    I2C_TransferSeq_TypeDef i2c_data;  
    i2c_data.addr = add;
...
static void Tca8418_Init_IO(void)
{
    UINT32 reg = REG_CFG;
    UCHAR flag = CFG_INT_CFG | CFG_OVR_FLOW_IEN | CFG_KE_IEN;
        trans_return_value = I2c0_write(TCA8418_ADDRESS, REG_CFG, CFG_INT_CFG | CFG_OVR_FLOW_IEN | CFG_KE_IEN); 
        trans_return_value = I2c0_write(TCA8418_ADDRESS, REG_KP_GPIO1, 0xFF); 
        trans_return_value = I2c0_write(TCA8418_ADDRESS, REG_KP_GPIO2, 0xFF); 
        trans_return_value = I2c0_write(TCA8418_ADDRESS, REG_KP_GPIO3, 0x03);      

#ifdef __TCA8418_DEBOUNCE__
        trans_return_value = I2c0_write(TCA8418_ADDRESS,REG_DEBOUNCE_DIS1,0xFF); 
        trans_return_value = I2c0_write(TCA8418_ADDRESS,REG_DEBOUNCE_DIS2,0xFF);         
        trans_return_value = I2c0_write(TCA8418_ADDRESS,REG_DEBOUNCE_DIS3,0x03); 
#endif
}

static void HAL_Process_TCA8418_Value(UINT32 keyNumber)
{      
        UCHAR i= 0 ,keypressindex = 0,keyReleaseIndex = 0;
        UCHAR keyPress[10]={0};
        UCHAR keyRelease[10]={0};
       for(;i<keyNumber;i++)
        {
               if(KEY_EVENT_VALUE&trans_data[i])
                {
                    keyPress[keypressindex] = trans_data[i];
                    keypressindex++;
                }
               else
                {
                    keyRelease[keyReleaseIndex] = trans_data[i];
                    keyReleaseIndex++;
                }
        }
       while(lastkey>0)
        {
            keyPress[keypressindex] =lastkey;
            keypressindex ++;
            lastkey = lastkey>>8;
        }

        lastkey = 0;
       for(i=0;i<keypressindex;i++)
        {
            UCHAR j = 0;
            for(;j<keyReleaseIndex;j++)
            {
                if((keyPress[i]&keyRelease[j]) == keyRelease[j])
                {
                    keyPress[i] = 0;
                }
            }
...
static UINT32 HAL_Key_TCA8418_Get_Value(void)
{
    ...
    UINT32 ret_key = 0;
    UCHAR clearFlag = 0xFF;
    UCHAR readValue;
    unsigned char  Key_I2C_Array[10][2];
    
    trans_return_value = I2c0_read(TCA8418_ADDRESS, REG_INT_STAT,&readValue);
    ...
    if( readValue & INT_STAT_OVR_FLOW_INT)
    {
        ret_key = 0;
        goto exit;
        //overflow occurred
    }
    if(readValue & INT_STAT_K_INT)
    {
        //read key value        
        for(i=0;i<10;i++)
        {
            trans_return_value = I2c0_read(TCA8418_ADDRESS, REG_KEY_EVENT_A,&readValue);
            trans_data[i] = readValue;          
            if(trans_data[i] == 0)
                break;
        }       
    }
    ret_key = i;
 exit:
   trans_return_value = I2c0_write(TCA8418_ADDRESS, REG_INT_STAT,CLEAR_INT_FLAG);
return ret_key;

}

static UINT32 Tca8418_Calc_Key(void)
{
    UINT32 ret_key = 0xAAAAAAAA;
    char row = (char)(Stop_Position>>8);
    char column = (char)(Stop_Position&0x00FF);
    UCHAR Key_Count = 0;
    UCHAR key[4] = {0};
    UCHAR findstopkey = 0;
    UINT32 tempkey = lastkey;
    UCHAR i = 0;
    unsigned char  Key_I2C_Array[2][2];
    while(tempkey>0)
    {
        key[Key_Count ] = tempkey;
        tempkey  = tempkey >>8;
        if(key[Key_Count ] == 0)
          continue;        
        if(Tca8418_Table[row][column] == (key[Key_Count ]&KEY_EVENT_CODE ))
        {
            findstopkey = 1;
        }
        Key_Count++;  
    ...
    if((Key_Count>1)&&(findstopkey))
    {
            ret_key = ((UINT32)(row<<24));
            ret_key |= ((UINT32)(column<<16));
            ret_key |= ((UINT32)(0xAA<<8));
            ret_key |= ((UINT32)(0x00));
    ...
    for(;i<Key_Count;i++ )
    {
        ...
        UCHAR r,c;
                ...
                r = code / TCA8418_MAX_COLS;
                c = code % TCA8418_MAX_COLS;
        
                r = (c) ? r : r - 1;
                c = (c) ? c - 1 : TCA8418_MAX_COLS - 1;
                Key_I2C_Array[i][0]= c;
                Key_I2C_Array[i][1] = r;
                //i++ ;
    ...
    ret_key = 0;
    ret_key = ((UINT32)Key_I2C_Array[0][0])<<24;
    ret_key |= ((UINT32)Key_I2C_Array[0][1])<<16;
    ret_key |= ((UINT32)Key_I2C_Array[1][0])<<8;
    ret_key |= ((UINT32)Key_I2C_Array[1][1]);
exit:
    if(keyi2c != ret_key)
    {
...
void HAL_Key_Initial_IO(PHYKEYMODE mode)
{
    ...
    else
    {
        ...
        if(I2cKeytype == SN7326)
        {
            trans_return_value = I2c0_write(SN7326_ADDRESS, 0x08,0x34);
        ...
        if((i2cTransferNack == trans_return_value)||(I2cKeytype == TCA8418) )
        {
            if(I2cKeytype  != TCA8418 )
            {
                I2cKeytype = TCA8418;
            }
            Tca8418_Init_IO();
...
#if 1
            if(HAL_Get_IO(I2CKEY_INT_PORT, I2CKEY_INT_PIN ) == 0)
            {
                trans_return_value = I2c0_write(TCA8418_ADDRESS, REG_INT_STAT,CLEAR_INT_FLAG);
            }
            #endif
...
UINT32 HAL_Key_Get_Value(PHYKEYMODE mode)
{
    ...
    else
    {
        if(I2cKeytype == TCA8418)
        {
            return HAL_Key_TCA8418_Get_Value( );
        }
        else if(I2cKeytype == SN7326)
        {
...
UINT32 Hal_Get_KeyValue(void)
{
    ...
    else if(I2cKeytype == TCA8418)
    {
         keyi2c=Tca8418_Calc_Key();
         ret = keyi2c;
    }
	return ret;
}
static void HAL_Key_I2C_IntISR(void)
{
    ...
    if(HAL_Get_IO_IntFlag(I2CKEY_INT_PIN))
    {
	   ...
	   keycheckcnt = 0 ;
        ...
        {
            HAL_Process_TCA8418_Value( keyv );
        }
        /*每个中断要求清零自己的中断标志*/
    }
}
