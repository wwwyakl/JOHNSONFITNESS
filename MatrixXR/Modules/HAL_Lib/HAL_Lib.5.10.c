...
static void Tca8418_Init_IO(void)
{
        ...
        trans_data[0] = REG_CFG;
        trans_data[1] = CFG_INT_CFG | CFG_OVR_FLOW_IEN | CFG_KE_IEN;
...
#if 1
        trans_data[0] = REG_DEBOUNCE_DIS1;
        trans_data[1] = 0xFF;
        ...
        trans_data[0] = REG_DEBOUNCE_DIS2;
        trans_data[1] = 0xFF;
        ...
        trans_data[0] = REG_DEBOUNCE_DIS3;
        trans_data[1] = 0x03;
        trans_return_value = I2C_TransferInit(I2C0,&i2c_data);
        while (trans_return_value == i2cTransferInProgress)
        {
...
{
	UINT32 ret ;
       if(((keycheckcnt >90)&&(Key_Releaseflag==0))&&(I2cKeytype == SN7326))
	{
            keyi2c = 0xAAAAAAAA;
		HAL_Key_I2C_Release( );
	}
...
static UINT32 HAL_Key_TCA8418_Get_Value(void)
{
    ...
    I2C_TransferReturn_TypeDef trans_return_value;
    ...
    if(trans_data[1] & INT_STAT_OVR_FLOW_INT)
    {
        ret_key = 0XAAAAAAAA;
        //overflow occurred
    }
    if(trans_data[1] & INT_STAT_K_INT)
    {
        //read key value        
        for(i=0;i<10;i++)
        {
            trans_data[0] = REG_KEY_EVENT_A;
            i2c_data.buf[1].data = &trans_data[i+1];
            trans_return_value = I2C_TransferInit(I2C0,&i2c_data);  
            while (trans_return_value == i2cTransferInProgress)
            {
                trans_return_value = I2C_Transfer(I2C0);
	...
	if((((!(trans_data[1] & KEY_EVENT_VALUE))&& !((trans_data[2] & KEY_EVENT_VALUE)))&&(Key_Count == 2) )
        ||((!(trans_data[1] & KEY_EVENT_VALUE))&&(Key_Count == 1))
        )
    {
		HAL_Key_I2C_Release( );
        ret_key = 0XAAAAAAAA	;
	...
	if(Key_Count > 2)
	{
		...
		{
			if((trans_data[j]& KEY_EVENT_CODE) == array_Table[row][column])
			{
	...
	while(i<Key_Count )
	{
        UCHAR code ;
        UCHAR r,c;
                code  = trans_data[i+1] & KEY_EVENT_CODE;
